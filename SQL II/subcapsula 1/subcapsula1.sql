...
INSERT INTO INSCRIPCION VALUES (14,102,51200, '2012-05-02 18:45:00');
  ...
  select carrera, 
		 count (legajo) as 'Cantidad de estudiantes' 
    from ESTUDIANTE 
 ...
 BETWEEN '1980-01-01' AND '1989-12-31';
  ...
  select (i.CURSO_codigo) as 'Curso N°', 
  		 count(*) as 'Cantidad de estudiantes' 
...
where i.CURSO_codigo = 101;
  ...
  SELECT c.nombre, 
  		 Count (*) as 'Cantidad de pasajeros' 
	FROM PASAJERO p INNER JOIN PAIS c 
    				ON p.idpais = c.idpais 
GROUP by c.pais;

  SELECT SUM (monto) AS 'Suma de todos los pagos' 
  	FROM PAGO;

  SELECT (pa.nombre) as 'Pasajero', 
  		 ROUND (SUM (p.monto),2) as 'Suma de pagos' 
	from PAGO p INNER JOIN PASAJERO pa 
    			ON p.idpasajero = pa.idpasajero 
GROUP BY pa.idpasajero;

  SELECT (pa.nombre) as 'Pasajero', 
  		 ROUND (SUM (p.monto),2) as 'Suma de pagos' 
	from PAGO p INNER JOIN PASAJERO pa 
    			ON p.idpasajero = pa.idpasajero 
where pa.idpasajero = 1;

  SELECT (pa.nombre) AS 'Pasajero', 
  		 ROUND (AVG (p.monto),2) as 'Promedio de pagos' 
    from PAGO p INNER JOIN PASAJERO pa 
    			ON p.idpasajero = pa.idpasajero 
GROUP BY pa.idpasajero;
